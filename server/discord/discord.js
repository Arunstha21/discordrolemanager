    ...
    try {
        ...
        if (channelKeys.length > 0) {
            for (const key of channelKeys) {
                const category = guild.channels.cache.find(c => c.type === 4 && c.id === key);
                if (!category) {
                ...
                for (const channel of channels) {
                    let existingRole = guild.roles.cache.find(r => r.name === channel.name);
                    if (!existingRole) {
                    ...
                    const existingChannel = guild.channels.cache.find(c => c.parentId === key && c.name === toKebabCase(channel.name));
                    if (!existingChannel) {
